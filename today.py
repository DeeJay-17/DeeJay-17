...
USER_NAME = os.environ['USER_NAME'] # 'DeeJay-17'
...
def recursive_loc(owner, repo_name, data, cache_comment, addition_total=0, deletion_total=0, my_commits=0, cursor=None):
    ...
    request = requests.post('https://api.github.com/graphql', json={'query': query, 'variables':variables}, headers=HEADERS) # cannot use simple_request(), need to save the file before raising Exception
    if request.status_code == 200:
...
def loc_query(owner_affiliation, comment_size=0, force_cache=False, cursor=None, edges=[]):
    """
    Uses GitHub's GraphQL v4 API to query all repositories accessible (with respect to owner_affiliation)
...
if __name__ == '__main__':
    """
    Deepanshu Jain (DeeJay-17)
    Adapted from Andrew Grant's (Andrew6rant) README generator
    """
    print('Calculation times:')
    # define global variable for owner ID and calculate the user's account creation date
    user_data, user_time = perf_counter(user_getter, USER_NAME)
    OWNER_ID, acc_date = user_data
    formatter('account data', user_time)
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2003, 5, 17))
